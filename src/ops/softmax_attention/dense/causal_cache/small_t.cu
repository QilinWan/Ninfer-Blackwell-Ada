...
void causal_attention_small_t_launch(
    ...
    if (cache.storage == KvCacheStorage::RK4V4E8) {
        causal_attention_small_t_e8_launch(q, k, v, pos, valid_columns, table_rows, scale, cache,
                                           envelope, column_begin, width, partial_acc, partial_m,
                                           partial_l, out, stream);
        return;
    }
...
void causal_attention_cached_small_t_launch(const Tensor& q, const Tensor& pos, float scale,
    ...
    if (cache.storage == KvCacheStorage::RK4V4E8) {
        causal_attention_cached_small_t_e8_launch(q, pos, scale, cache, envelope, partial_acc,
                                                  partial_m, partial_l, out, stream);
        return;
    }
