// ninfer::ops::detail - E8 Conway-Sloane K / i4 V append launch ownership (Ada-only route).
#include "ops/kv_cache/append/launch.h"

#include "core/device.h"
#include "ops/common/math.h"
#include "ops/kv_cache/append/e8_kernel.cuh"
...
void launch_e8_for(const Tensor& k, const Tensor& v, const Tensor& positions, CacheView cache,
    ...
    if (tokens >= 128 && Geometry::KVHeads == 2) {
        ...
        kv_cache_append_full_e8_page_kernel<Geometry, Metadata><<<grid, kBlock, 0, stream>>>(
        ...
        kv_cache_append_full_e8_kernel<Geometry, Metadata><<<grid, kBlock, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(k.data), static_cast<const __nv_bfloat16*>(v.data),
            static_cast<const std::int32_t*>(positions.data), metadata, cache_k, cache_v, scale_k,
            scale_v, tokens);
    }
...
void dispatch_e8(const Tensor& k, const Tensor& v, const Tensor& positions, CacheView cache,
                 Metadata metadata, cudaStream_t stream) {
    if (k.ne[1] == KVCacheAppendD256Kv4::KVHeads) {
        launch_e8_for<KVCacheAppendD256Kv4>(k, v, positions, cache, metadata, stream);
        return;
    }
    launch_e8_for<KVCacheAppendD256Kv2>(k, v, positions, cache, metadata, stream);
}

} // namespace

void kv_cache_append_e8_launch(const Tensor& k, const Tensor& v, const Tensor& positions,
                               PagedKVLayerView cache, cudaStream_t stream) {
    const PagedKVDirectMetadata metadata{static_cast<const std::int32_t*>(cache.block_table.data)};
    dispatch_e8(k, v, positions, cache, metadata, stream);
}

void kv_cache_append_e8_batch_launch(const Tensor& k, const Tensor& v, const Tensor& positions,
