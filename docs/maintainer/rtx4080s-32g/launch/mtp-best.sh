#!/usr/bin/env bash
# ============================================================================
#  qwen3.8-27b-lynn-mtp.ninfer  ·  NInfer (Ada sm_89)  ·  RTX 4080 SUPER 32GB
#  Tuned 2026-09-24 on autodl-container-84a24c9a34-7083e900 (driver 595.71.05)
#
#  Measured single-stream decode (greedy, 512 new tokens, code prompt):
#      --spec mtp --draft-tokens 3 --lm-head-draft   ~88 tok/s  (accept 58%, len 2.73)
#      --spec mtp --draft-tokens 3   (no lm-head)    ~81 tok/s
#      --spec mtp --draft-tokens 5 --lm-head-draft   ~78 tok/s
#      no speculation                                ~40 tok/s
#      => MTP only supports K=1..5; K=3 is both the trained window and the optimum
#  Prefill (8K-token prompt): ~1.4-1.5k tok/s (chunk has almost no effect on Ada)
#
#  Usage:
#    ./mtp-best.sh                      # one-shot, 1024 tokens
#    ./mtp-best.sh ask "your question"
#    ./mtp-best.sh serve                # OpenAI/Anthropic HTTP server on :8080
#    MODE=think ./mtp-best.sh ask "hi"      # Qwen thinking preset sampling
#    CTX=131072 KVCAP=262144 LANES=2 ./mtp-best.sh serve   # 2 lanes, 256K shared pool
#
...
ARTIFACT=${ARTIFACT:-/root/autodl-tmp/models/qwen3.8-27b-lynn-mtp.ninfer}
...
KV=${KV:-int8}          # int8 beats fp8 by ~2%; nvfp4/k8v4 are Blackwell-only
SPEC=${SPEC:-mtp}       # this artifact carries the MTP head
DRAFT=${DRAFT:-3}       # MTP accepts K=1..5 only; 3 is trained window and optimum
LMD=${LMD:-1}           # --lm-head-draft: +9% at K=3 (the proposal head is worse)
CHUNK=${CHUNK:-2048}    # prefill chunk (multiple of 128)
...
# think   : Qwen3.8 thinking preset  (T=1.0  top-p=.95 top-k=20 min-p=0)
# nothink : Qwen3.8 non-thinking preset (T=0.7 top-p=.80 top-k=20 min-p=0, presence 1.5)
case "$MODE" in
  fast)  FLAGS+=(--greedy) ;;
